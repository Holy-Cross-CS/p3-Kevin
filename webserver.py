...
# Updated: September 2024 by Kevin Graziosi
#          - Changed handle_http_get_hello() to be html formatted. Also made it
#            so it is personalized if the user puts in their name and fav color
#          - Changed handle_http_get_file() to return different mime types based
#            on the given url
#          - Added an index.html file to the web_root
#          - Changed handle_http_get() so if the request is GET / then it gets
#            /index.html instead
# Updated: October 2024 by Kevin Graziosi
#          - Added handle_http_get_topics(), this is for whisper functionality
#            and it lets the topics and their information get sent to the client
#          - Added handle_http_posst(), this is for whisper functionality so the
#            client can post onto the whisper web app
#          - Added handle_http_post_msg()
#          - Added handle_http_post_likes()
#          - Added http_handle_get_feed()
...
class Statistics:
    def __init__(self):
        ...
        self.num_times_visit_hello = 0 # num of times visited the hello page

        # I wasn't really sure where else to put these, so I put them here
        self.topic_list_ver_num = 0
        # a list of lists, each list is structured ['topic', num_msgs, num_likes, ver_num]
        self.topic_list = [['holycross', 2, 0, 0], ['whatever', 2, 0, 0]]
        self.all_messages = [['test msg with #holycross', 'another #holycross test msg'], ['livin life #whatever', 'crazy hair dont care! #whatever']]
stats = Statistics()
...
def handle_http_get_hello(req, conn):
    ...
    name = ''
# checks if it's a personalized GET /hello, or generic GET /hello
# if it is the GET /hello? then it splits up the path to get the 
# username and favorite color
    if req.path.startswith('/hello?'):
        temp = req.path.split("?")
        username_fav_split = temp[1].split('&')
        if username_fav_split[0].startswith('username'):
            username_split = username_fav_split[0].split('=')
            temp3 = username_split[1].split('+')
            for i in range(0, len(temp3)):
                s = urllib.parse.unquote(temp3[i])
                name = name + s + ' '
            favcolor_split = username_fav_split[1].split('=')
            favcolor = favcolor_split[1]
        elif username_fav_split[0].startswith('favorite'):
            favcolor_split = username_fav_split[1].split('=')
            favcolor = favcolor_split[1]
            username_split = username_fav_split[1].split('=')
            temp3 = username_split[1].split('+')
            for i in range(0, len(temp3)):
                s = urllib.parse.unquote(temp3[i])
                name = name + s + ' '
    ...
    with stats.lock:
        stats.num_times_visit_hello += 1
    if favcolor not in [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" ]:
        favcolor = None
    msg = f"<html><head><title>Welcome, {name}!</head></title>\n"
    msg += '<body>'
    msg += '<p>The current date is %s.\n</p>' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += '<p> Hello %s.</p>' % (name)
    with stats.lock:
        msg += '<p>This is your %d time visiting this page.\n</p>' % (stats.num_times_visit_hello)
    msg += f'<p>Maybe your favorite color is... {favcolor}??\n</p>'
    msg += '<p>Hit page refresh (F5) to refresh this page and see the contents MOSTLY STAY THE SAME??\n</p>'
    msg += '<p>You can also go to these exciting pages:\n</p>'
    msg += f'  <p><a href="/hello">http://{server_host}:{server_port}/hello</a>       - this page</p>'
    msg += f'  <p><a href="/quote">http://{server_host}:{server_port}/status</a>       - status and statistics</p>'
    msg += f'  <p><a href="/welcome.html">http://{server_host}:{server_port}/quote</a>        - a random quote</p>'
    msg += f'  <p><a href="/welcome.html">http://{server_host}:{server_port}/welcome.html</a> - a welcome page</p>'
    msg += f'  <p><a href="/index.html">http://{server_host}:{server_port}/index.html</a>   - some fancy web page</p>'
    msg += "\n"
    msg += '<p>Please use the following form to change your name or favorite color:</p>'
    msg += '<form method="GET" action="/hello"'
    msg += 'style="background: gray; padding: 5px; margin: 5px; display: inline-block;">'
    msg += 'Enter your name: <input type="text" name="username" value="van Gogh"><br>'
    msg += 'Favorite color: <select name="favorite">'
    msg += '<option value="coral">A nice Coral</option>'
    msg += '<option value="crimson">A nice Crimson</option>'
    msg += '<option value="navy">A nice Navy</option>'
    msg += '<option value="purple">A nice Purple</option>'
    msg += '<option value="tomato">A nice Tomato</option>'
    msg += '<option value="teal">A nice Teal</option>'
    msg += '<option value="wheat">A nice Wheat</option>'
    msg += '</select>'
    msg += '<input type="submit" value="Greet Me!"></form>'
    msg += '</body></html>'
...
# handle_http_get_topics() returns an appropriate response for a GET request that 
# is for the topics list. 
def handle_http_get_topics(req):
    log("Handling http get topics request")
    temp = req.path.split('=')
    req_ver_num = int(temp[1])
    with stats.lock:
        while stats.topic_list_ver_num < req_ver_num:
            stats.lock.wait()
        msg = str(stats.topic_list_ver_num) + '\n'
        for i in range(0, len(stats.topic_list)):
            msg += str(stats.topic_list[i][1]) + ' ' + str(stats.topic_list[i][2]) + ' ' + stats.topic_list[i][0] + '\n'
    return Response('200 OK ', 'text/plain', msg)

# handle_http_get_feed() returns all of the messages for a given topic
def handle_http_get_feed(req):
    log('Handling http get feed request')
    temp = req.path.split('?')
    topic = temp[0].split('/')
    if len(topic) != 4:
        return Response('400 BAD REQUEST ', 'plain/text', 'Please include a topic to get the feed of\n')
    topic = topic[3]
    ver_num = temp[1].split('=')
    ver_num = int(ver_num[1])
    index = 0
    with stats.lock:
        for i in range(0, len(stats.topic_list)):
            if stats.topic_list[i][0] == topic:
                index = i
                break
        while stats.topic_list[index][3] < ver_num:
            stats.lock.wait()
        msg = str(ver_num) + '\n'
        for i in range(0, len(stats.all_messages[index])):
            msg += '- ' + stats.all_messages[index][i] + '\n'
    return Response('200 OK ', 'plain/text', msg)
...
def handle_http_post(req):
    if req.path == "/whisper/messages":
        ...
        resp = handle_http_post_likes(req)
    return resp

# handle_http_post_msg() takes the body of the clients request and uses that info
# to update the relevant variables in the server
def handle_http_post_msg(req):
    log("Handling post message request")
    tags = ''
    msg = ''
    i = 0
    in_or_out = 0
    while not tags.endswith('\n'):
        tags += req.body[i]
        i += 1
    ...
    with stats.lock:
        for i in range(1, len(temp)):
            for j in range(0, len(stats.topic_list)):
                if stats.topic_list[j][0] == temp[i]:   # if the topic exists, +1 to msg count
                    stats.topic_list[j][1] += 1
                    stats.topic_list[j][3] += 1         # and add one to the ver num for the topic
                    stats.lock.notify_all()
                    stats.all_messages[j] += [msg]      # trying to make it add the msg to the msg list
                    in_or_out = 1
            if in_or_out == 0:
                stats.topic_list += [[temp[i], 1, 0, 0]]
                stats.all_messages += [[msg]]
        stats.topic_list_ver_num += 1
        stats.lock.notify_all()
    return Response('200 OK ', 'text/plain', 'success')
        
# handle_http_post_likes() takes the path of the clients request and uses that info
# to update the like count for that topic
def handle_http_post_likes(req):
    log('Handling post like request')
    with stats.lock:
        for i in range(0, len(stats.topic_list)):
            if req.path.endswith(stats.topic_list[i][0]):
                stats.topic_list[i][2] += 1
                stats.topic_list[i][3] += 1
                stats.lock.notify_all()
                return Response('200 OK ', 'text/plain', 'success')
    return Response('404 NOT FOUND', 'plain/text', 'The topic you tried to like does not exist')
